...
1. **Heed Compiler Warnings**: Compile code with high warning levels and resolve all warnings.
1. **Architect and Design for Security Policies**: Ensure the software architecture supports policy enforcement.
1. **Keep It Simple**: Keep designs simple to make verification and auditing easier.
1. **Default Deny**: Restrict access by default and grant permissions explicitly.
1. **Adhere to the Principle of Least Privilege**: Run code with the minimum set of privileges necessary.
1. **Sanitize Data Sent to Other Software**: Prevent command injection attacks on database and operating systems.
1. **Practice Defense in Depth**: Implement multiple layers of security controls.
1. **Use Effective Quality Assurance Techniques**: Conduct fuzz testing, static analysis, and manual code reviews.
1. **Adopt a Software Construction Security Standard**: Adhere to an established secure coding standard.
