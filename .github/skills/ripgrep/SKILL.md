---
description: Explains how to use Ripgrep (rg), including common flags, context options, file filtering.
name: ripgrep
---
...
```bash
rg -i "my-search-term" packages/agents-build/src/
```
...
```bash
rg -C 3 "function complexLogic" src/
```
...
```bash
rg -l "export const globalRules" packages/
```
...
```bash
rg --no-ignore "build_artifact" dist/
``
