...
export const engineeringFoundationsModelingSimulation = defineRule({
  content: `# Engineering Foundations - Modeling, Simulation, and Prototyping
...
### 1.1 Modeling Concepts and Typologies
...
- **Key Components of Discrete Simulation**: A discrete simulation model contains several essential elements:
...
- **Software Prototyping Strategies**:
...
## 2. Modeling, Simulation, and Prototyping Compliance Checklist
This checklist defines SWEBOK-derived criteria for verifying the quality and methodology of modeling, simulation, and prototyping activities:

- [ ] **Model Abstraction Level**: Is the model clearly classified (iconic, analogic, or symbolic), and is the level of abstraction appropriate for the design concern?
- [ ] **Simplification Assumptions**: Are all simplifications, omissions, and assumptions made during model construction explicitly documented?
- [ ] **Simulation Type Selection**: Is the choice between discrete and continuous simulation aligned with the behavioral characteristics of the target system?
- [ ] **Discrete Component Identification**: In discrete simulations, are all entities, activities, events, resources, and state variables clearly mapped?
- [ ] **Initialization Bias Mitigation**: Is the simulation's initialization state documented, and is a warm-up period defined to mitigate initialization bias?
- [ ] **Random Number Verification**: Is the pseudo-random number generator verified for statistical independence and period length to ensure simulation validity?
- [ ] **Simulation Replications**: Are multiple replication runs conducted with different random seeds to establish statistical confidence intervals for the outputs?
- [ ] **Prototype Goal Definition**: Does the prototype have a clearly defined, documented objective (e.g., requirements elicitation vs. technical feasibility)?
- [ ] **Quality Compromise Documentation**: Are the specific quality attributes (e.g., security, scalability) sacrificed in the software prototype documented?
- [ ] **Prototype Lifecycle Control**: Is the prototype's development planned, monitored, and controlled rather than built ad-hoc?
- [ ] **Disposition Strategy**: Is there a clear, pre-defined disposition plan for the prototype (e.g., throwaway prototype vs. evolutionary framework)?
- [ ] **Model-System Alignment**: Is the model or simulation validated against real-world empirical data to ensure it accurately represents the target system?
- [ ] **Subsystem Interaction Mapping**: Does the simulation model the relationships and dependencies between different subsystems?
- [ ] **Interface Consistency**: Do the simulated components interact using the same interface boundaries (APIs) defined in the system architecture?
- [ ] **Representational Completeness**: Does the model capture all relevant attributes of interest needed to make the design decisions under test?
- [ ] **Feedback Loop Integration**: Are the insights and validation results obtained from the prototype or simulation fed back into the requirements and design baselines?`,
  description:
