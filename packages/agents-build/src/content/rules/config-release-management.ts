...
export const configReleaseManagement = defineRule({
  content: [
    {
      level: 1,
      text: "Configuration Release Management and Delivery",
      type: "header"
    },
    {
      level: 2,
    ...
    {
      level: 3,
      text: "1.4 Software Configuration Management Tools and CMDB",
      type: "header"
    },
    {
    ...
    {
      text: "The following checklist items are aligned with SWEBOK v4 configuration release management and delivery standards:",
      type: "text"
    },
    {
      items: [
        {
          text: "Are all compilation, linking, and packaging settings version-controlled alongside the source code?"
        },
        {
          text: "Is the build environment, including specific compiler and interpreter versions, documented and reproducible?"
        },
        {
          text: "Does every build produce a Software Bill of Materials (SBOM) listing all dependencies and transitive libraries?"
        },
        {
          text: "Are release notes generated for each distribution, outlining new features, resolved issues, and runtime platform requirements?"
        },
        {
          text: "Does the project utilize a Version Description Document (VDD) to list the physical files and checksums of each release package?"
        },
        {
          text: "Are release decisions based on quantitative criteria, such as known defect severity and prior release fault densities?"
        },
        {
          text: "Are installation and data migration scripts verified for compatibility against legacy versions?"
        },
        {
          text: "Is the integrity of the release package protected using cryptographic hashes or digital signatures?"
        },
        {
          text: "Does the continuous integration pipeline execute automatically on every commit to the main code branch?"
        },
        {
          text: "Are static analysis, unit tests, and code coverage checks integrated as blocking gates in the build pipeline?"
        },
        {
          text: "Are built binary files and libraries stored in a versioned repository to allow independent release verification?"
        },
        {
          text: "Does the configuration management system trace release contents back to the specific change requests that authorized them?"
        },
        {
          text: "Are software variants and platform-specific packages constructed using automated configuration parameters rather than manual edits?"
        },
        {
          text: "Does the configuration database record the association between logical releases and the environments they are deployed in?"
        },
        {
          text: "Are the SCM tools chosen and scaled to match the organization's requirements for parallel development, branch management, and certification?"
        },
        {
          text: "Is there an automated process to check for third-party dependency updates and license compliance in the build?"
        },
        {
          text: "Are build instructions detailed, sequential, and free of dependency on undocumented developer environment variables?"
        },
        {
          text: "Is workspace management structured to prevent developer modifications from overriding the master baseline without code review?"
        },
        {
          text: "Are verification audits performed on release candidates to confirm that all functional requirements are satisfied?"
        }
      ],
      type: "unorderedList"
    }
