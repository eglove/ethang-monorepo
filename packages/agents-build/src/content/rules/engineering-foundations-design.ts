...
export const engineeringFoundationsDesign = defineRule({
  content: `# Engineering Foundations - Engineering Design
...
2. **Implicit Constraints**: Non-negotiable limits imposed by the laws of nature, physics, and logic. In physical engineering, these include the strength of materials, thermodynamics, and electromagnetism. In software engineering, these include computational complexity (Big O limits), network latencies, concurrency limits, and hardware resource boundaries.
3. **State of Knowledge Constraints**: The limitations of the engineering discipline's current state of the art, which restrict the tools, materials, and methods that can be reliably employed.
