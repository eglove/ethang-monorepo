...
export const economicsEstimation = defineRule({
  content: `# Software Engineering Economics: Estimation
...
2. Identify a suitable historical analogy for which accurate actual results are known.
3. List the structural, functional, or technical differences between the new target and the historical analogy.
4. Estimate the physical or complexity magnitude of each identified difference.
5. Calculate the final estimate by adjusting the historical actuals by the calculated differences.
...
2. Estimate the effort, size, or cost for each of the lowest-level components.
3. Sum the individual estimates to construct the aggregate estimate for the entire system.
4. Apply explicit adjustment factors to account for cross-cutting tasks (such as requirements engineering, integration, system testing, and documentation) that are not captured at the individual component level.
