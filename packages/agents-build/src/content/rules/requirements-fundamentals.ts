...
### 1.6 Quality of Service (QoS) Constraints
...
### 1.7 Why Categorize Requirements This Way?
SWEBOK v4 outlines several key reasons for categorizing requirements:
- Requirements in different categories tend to originate from different stakeholder sources.
- Elicitation, analysis, and specification techniques vary significantly by category.
- Different validation authorities are responsible for validating different requirements categories.
- The different categories impact the resulting software architecture and code in distinct ways.
...
### 1.12 Software Requirements Activities
The software requirements process is split into two primary operational areas:
- Requirements Development: The activities of Elicitation (capturing candidate requirements), Analysis (evaluating consistency, feasibility, and economics), Specification (documenting requirements for communication), and Validation (confirming requirements represent true stakeholder needs).
- Requirements Management: The activities of Requirements Scrubbing (pruning obsolete requirements), Change Control (managing modifications to the baseline), and Scope Matching (balancing product features with cost, schedule, and staffing constraints).

## 2. Compliance Checklist
- [ ] Has each requirement been classified as either a Software Product Requirement or a Software Project Requirement?
- [ ] Are the functional requirements clearly separated from nonfunctional requirements using the Perfect Technology Filter?
- [ ] Have the nonfunctional requirements been partitioned into Technology Constraints (mandating/prohibiting named platforms) and Quality of Service (QoS) Constraints (specifying performance levels)?
- [ ] Is there an explicit mapping showing how system-level requirements allocate and translate to specific software-level requirements?
- [ ] Are all derived requirements arising from architectural or design decisions documented and traced back to their parent decisions?
- [ ] Has recursive design been applied to decompose parent QoS constraints into child functional requirements?
- [ ] Are all Quality of Service constraints specified quantitatively (e.g., using response time, throughput, or accuracy parameters)?
- [ ] Has a clear boundary been established between the software requirements and the hardware/platform environment?
- [ ] Do the functional requirements focus strictly on business policies to be enforced and processes to be carried out?
- [ ] Have requirements development activities (elicitation, analysis, specification, validation) been planned and tailored to the project context?
- [ ] Have requirements management activities, including change control and scope matching, been defined to maintain the agreement baseline over time?`,
  description:
