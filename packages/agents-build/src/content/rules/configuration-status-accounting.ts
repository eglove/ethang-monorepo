...
export const configurationStatusAccounting = defineRule({
  content: [
    {
      level: 1,
      text: "Configuration Status Accounting",
      type: "header"
    },
    {
      level: 2,
    ...
    {
      level: 3,
      text: "1.5 Configuration Status Metrics and Quantitative Analysis",
      type: "header"
    },
    {
      text: "Quantitatively measuring configuration stability and change velocity allows engineering teams to identify process bottlenecks and quality risks. SCSA supports this by generating metrics such as:",
      type: "text"
    },
    {
      items: [
        {
          text: "**Change Request Lead Time ($$LT_{CR}$$)**: The average duration required to implement and verify an approved change request.\n$$LT_{CR} = \frac{1}{N} sum_{i=1}^{N} (T_{    ext{completed}, i} - T_{    ext{approved}, i})$$\nWhere $T_{    ext{completed}}$ is the timestamp of final verification and merge, and $T_{    ext{approved}}$ is the timestamp of formal change approval."
        },
        {
          text: "**Change Request Density per SCI ($$CRD_{SCI}$$)**: The count of Change Requests (CRs) directed at a specific Software Configuration Item (SCI), highlighting unstable components.\n$$CRD_{SCI} = \frac{N_{    ext{CR}}}{N_{    ext{SCI}}}$$"
        },
        {
          text: "**Baseline Integrity Index ($$BII$$)**: The ratio of verified configuration items matching their registered cryptographic hashes to the total number of items in the baseline.\n$$BII = \frac{N_{    ext{verified}}}{N_{    ext{total}}}$$\nBy analyzing these metrics, organizations can evaluate process maturity and identify components that suffer from high change volatility or high defect density."
        }
      ],
      type: "unorderedList"
    },
    ...
    },
    {
      items: [
        {
          text: "Are all configuration items (CIs) uniquely identified and tracked with their ongoing and approved status?"
        },
        {
          text: "Is the current implementation status of each approved change request recorded and visible?"
        },
        {
          text: "Are the relationships and dependencies among CIs systematically mapped and recorded to determine change impact?"
        },
        {
          text: "Are all deviations and waivers from approved configurations formally documented, authorized, and tracked?"
        },
        {
        ...
        {
          text: "Are security status indicators, including governance, risk, and compliance records, associated with each configuration status audit?"
        },
        {
          text: "Is the number of change requests (CRs) associated with each Software Configuration Item (SCI) tracked and reported?"
        },
        {
          text: "Is the average implementation time (lead time) for change requests calculated and monitored for process bottlenecks?"
        },
        {
          text: "Are status accounting records secured against unauthorized modifications to prevent tampering of historical configuration states?"
        },
        {
          text: "Is the status of physical and functional audits recorded and linked to the corresponding baseline validation reports?"
        },
        {
          text: "Are configuration status reports generated regularly to satisfy quality assurance, legal, and regulatory compliance requirements?"
        },
        {
          text: "Does the status accounting system support ad hoc queries to retrieve the historical lineage and configuration evolution of any CI?"
        },
        {
          text: "Is there verification evidence (e.g., test execution runs, requirement completion matrices) linked to each release baseline?"
        },
        {
          text: "Are configuration states (e.g., proposed, in-progress, verified, released) defined by clear transition rules?"
        },
        {
          text: "Are build configurations and release packaging details accounted for to guarantee repeatable deployments?"
        },
        {
          text: "Are status accounting records integrated with change control records to prevent untracked updates?"
        }
      ],
      type: "unorderedList"
    }
  ...
  filename: "configuration-status-accounting",
