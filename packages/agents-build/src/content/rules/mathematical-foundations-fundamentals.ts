...
export const mathematicalFoundationsFundamentals = defineRule({
  content: `# Mathematical Foundations: Basic Logic
...
2. **The Law of Contradiction**: For every proposition p, it is not the case that p is both true and false simultaneously.
...
2. **The Existential Quantifier (∃x)**: Asserts that a predicate P(x) is true for at least one value of the variable x in its domain. For example, ∃x (Server(x) ∧ Overloaded(x)) asserts that there exists at least one server that is overloaded. Existential quantification naturally pairs with conjunction.
