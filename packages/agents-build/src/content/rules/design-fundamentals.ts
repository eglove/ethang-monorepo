...
export const designFundamentals = defineRule({
  content: `# Software Design Fundamentals
...
2. **No Stopping Rule**: There is no objective criteria for completion; design ends due to constraints like budget, time, or resource depletion.
3. **Qualitative Solutions**: Solutions are not binary (correct/incorrect), but rather qualitative (better, worse, or sufficient).
4. **Shifting Constraints**: The problem definition changes dynamically as the solution is explored, reacting to external shifts.
5. **One-Shot Operations**: Every implemented solution is consequential and cannot be easily undone without significant cost.
...
2. **Formulate a Concept**: Devise a high-level conceptual model or architectural paradigm describing how the purpose can be achieved.
3. **Devise a Mechanism**: Select or construct the concrete structural entities (classes, components, or services) and behavioral patterns that realize the conceptual structure.
4. **Introduce a Notation**: Establish a formal vocabulary and notation (such as APIs, TypeScript interfaces, and schemas) to express the capabilities of the mechanism and invoke its use.
5. **Describe the Usage of the Notation**: Map specific problem instances and scenarios to the notation to trigger the underlying mechanism, thereby solving the purpose.
...
2. **Separation of Concerns (SoC)**: Identifying and isolating distinct areas of interest so that designers can focus on each concern in isolation. Dijkstra noted that separation of concerns is the only available technique for the effective ordering of one's thoughts.
3. **Modularization and Decomposition**: Structuring large software into smaller, named components with well-defined interfaces. David Parnas advocated that each module should have a single responsibility, making components easier to understand, construct, and maintain.
4. **Encapsulation and Information Hiding**: Restricting direct access to a module's internal state and implementation details. Encapsulation hides volatile design decisions (secrets) behind stable public interfaces.
5. **Separation of Interface and Implementation**: Defining components via public APIs and isolating the client from the details of how the component is built. This ensures internal changes do not affect external consumers.
6. **Coupling**: The measure of the degree of interdependence between modules. Designers strive for loose coupling to minimize the ripple effect of changes. SWEBOK v4 emphasizes that modules should be loosely or weakly coupled. The coupling scale ranges from Content, Common, Control, Stamp, Data, to Message coupling.
7. **Cohesion**: The measure of the strength of association of the elements within a module. Most design methods advocate that modules should maximize cohesion and localization, grouping elements based on their relatedness. The cohesion scale ranges from coincidental, logical, temporal, procedural, communicational, sequential, to functional cohesion.
8. **Uniformity**: Enforcing consistency in naming schemes, notations, syntax, and interface structures across all components. This is achieved through conventions such as rules, formats, and styles.
9. **Completeness and Sufficiency**: Ensuring a design captures all essential characteristics of an abstraction, maps all states and modes, and demonstrates how requirements are met.
10. **Verifiability**: Structuring the design so that it can be audited, traced to requirements, and validated. This is critical for high-assurance systems with safety or security concerns.
11. **Ethically Aligned Design**: Addressing human values, transparency, accountability, data agency, and dependability in AI-enabled or autonomous systems. SWEBOK v4 highlights eight principles: Human Rights, Well-being, Data Agency, Effectiveness, Transparency, Accountability, Awareness of Misuse, and Competence.
