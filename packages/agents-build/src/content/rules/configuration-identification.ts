...
- **Functional Baseline**: Established at the completion of the system requirements phase, specifying the functional requirements that the system must satisfy.
- **Allocated Baseline**: Established after design, allocating requirements to specific hardware or software configuration items.
- **Product Baseline**: Established at the completion of system integration and testing, representing the compiled, verified, and deliverable state of the software.

### 1.5 Relationships Scheme Definition
Relationships provide the connections required to create and sustain structure. The ability to communicate intent and manage development results is enhanced when effective relationships (structuring) are in place. The status accounting activity is responsible for gathering information about relationships among CIs. SWEBOK v4 defines several common relationship types:
- **Dependencies**: CIs depend mutually or one-way on each other (e.g., a class model depends on a sequence diagram because a change to either affects the other).
- **Derivation**: One CI derives from another sequentially due to a constraint requiring one CI to be completed before the other is developed (e.g., source code deriving from design specifications).
- **Succession**: Software items evolve. A version is an identified instance or state of an evolving item. The succession relationship reflects this evolutionary progression and is reflexive. The first succession occurs when a CI is created; subsequent changes create new successions, which is how versions are tracked.
- **Variants**: Program versions resulting from engineered alternative options. Variants are less common because they are expensive to maintain.
- **Software Bill of Materials (SBOM)**: A formal record containing the details and supply chain relationships of the CIs (components) used in building software. Components can be source code, third-party libraries, modules, open-source or proprietary, and free or paid. The SBOM provides deep transparency to monitor vulnerabilities and licensing compliance across the software supply chain.

### 1.6 Software Libraries and Security
A software library is a controlled collection of source code, scripts, object code, documentation, and related artifacts. SCM relies on libraries to enforce access control, segregation of duties, and release management. SWEBOK v4 highlights the transition of artifacts through a sequence of library environments: developer libraries (working spaces with minimal control), controlled libraries (staging or integration areas with automated testing and build checks), and formal libraries (production release areas with strict access rules). Requirements and test cases should be linked with the code baselines. Source code is stored in version control, providing traceability and security. Multiple development streams are supported in version control linked to binary objects derived during builds. Binary objects are typically stored in a repository containing cryptographic hashes to perform the physical configuration audit (PCA). The definitive media library contains release baselines of artifacts deployable to test, stage, and production systems. In terms of access control and backup facilities, security is a key aspect of library management. Proper access control ensures only authorized processes promote artifacts, while automated backups prevent data loss.

## 2. Compliance Checklist
- [ ] **SCI Identification**: Are all relevant artifacts (plans, specifications, test materials, tools, source code, docs) selected as SCIs?
- [ ] **Visibility Balance**: Is there a documented balance in SCI selection to ensure manageable control without losing visibility?
- [ ] **System Context**: Is the software configuration defined and aligned within the context of the overall system configuration?
- [ ] **Standardized Identifier**: Does each SCI have a unique identifier using a documented significant or nonsignificant schema?
- [ ] **Attribute Tracking**: Does the SCM scheme track essential CI attributes (name, description, dates, type, owner) in status accounting?
- [ ] **Baseline Designation**: Are all baselines formally approved, designated, and associated with a specific date and time?
- [ ] **Baseline Attributes**: Are baseline attributes (name, identifier, description, creation date, CIs) recorded in status accounting?
- [ ] **Formal Baseline Gate**: Is there a check ensuring that once established, baselines can only be modified via formal change control?
- [ ] **Dependency Mapping**: Are dependency relationships among CIs mapped and tracked to analyze change impacts across modules?
- [ ] **Derivation Tracking**: Are derivation relationships tracked to maintain alignment of code with design and requirements?
- [ ] **Succession Records**: Are succession relationships recorded for each CI to document version history and creation dates?
- [ ] **Variant Assessment**: If variants are used, are their relationships documented and is their long-term maintenance cost evaluated?
- [ ] **SBOM Accuracy**: Is an SBOM maintained tracking all components, libraries, and modules, including licenses and dependencies?
- [ ] **VCS Integration**: Is source code stored in a version control system that supports multiple development streams and traceability?
- [ ] **Binary Hash Auditing**: Are compiled binaries associated with cryptographic hashes in a repository to support PCA validation?
- [ ] **DML Isolation**: Is a definitive media library used to isolate release baselines of artifacts from development areas?
- [ ] **Library Access Control**: Are promotion gates protected by access control policies to prevent unauthorized baseline changes?`,
  description:
