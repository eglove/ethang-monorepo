...
export const architectureDescription = defineRule({
  content: `# Software Architecture Description
...
2. **Component and Connector Viewpoint**: Captures the runtime organization of the system. It models execution elements (components, such as services, processes, and clients) and the communication pathways (connectors, such as remote procedure calls, message queues, and databases) through which they interact during operation, addressing concurrency, synchronization, and performance.
3. **Logical Viewpoint**: Models the fundamental concepts, business domain entities, and operational capabilities of the software. It maps how the system will satisfy the functional requirements of end-users.
4. **Scenarios and Use Cases Viewpoint**: Illustrates how users and external actors interact with the system, demonstrating how the architectural components collaborate to satisfy functional requirements. It acts as the "+1" that ties other views together.
5. **Information Viewpoint**: Details how key data entities are structured, processed, synchronized, and stored. It addresses concerns related to data consistency, transaction boundaries, and database constraints.
6. **Deployment Viewpoint**: Details the physical configuration of the software, mapping components to hardware nodes, edge runtimes, servers, and database hosts.
...
2. **Distributed Systems**: Includes client-server configurations, multi-tier (n-tier) designs, broker patterns (coordinating distributed communication), publish-subscribe, point-to-point networks, and Representational State Transfer (REST).
3. **Method-Driven Paradigms**: Includes object-oriented structures, event-driven designs (decentralized components reacting to asynchronous state events), and data flow architectures.
4. **User-Computer Interaction**: Includes Model-View-Controller (MVC) and Presentation-Abstraction-Control (PAC) styles, separating domain logic from user presentation.
5. **Adaptive Systems**: Includes microkernels and reflection/meta-level architectures that support runtime modification.
6. **Virtual Machines**: Includes interpreters, rule-based runtimes, and process control loops.
