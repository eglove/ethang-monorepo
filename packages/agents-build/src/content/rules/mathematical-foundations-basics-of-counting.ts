...
### 1.2 The Principle of Inclusion-Exclusion

When two or more tasks can be performed simultaneously, they represent non-disjoint sets. Applying the sum rule directly in this context results in double-counting the elements in their intersection. The Principle of Inclusion-Exclusion corrects this by subtracting the intersection:
|A union B| = |A| + |B| - |A intersection B|

For three sets, the principle includes the individual sets, excludes double intersections, and includes the triple intersection:
|A union B union C| = |A| + |B| + |C| - |A intersection B| - |A intersection C| - |B intersection C| + |A intersection B intersection C|

This principle is vital when optimizing database queries involving multiple search criteria with logical OR operations. It ensures that the database query optimizer accurately estimates the result set size (cardinality estimation) to choose the most efficient index.
...
### 1.4 Recursion and Recurrence Relations

Recursion is the process of defining an object, function, set, or algorithm in terms of itself. It is a powerful tool for solving complex problems by decomposing them into smaller, identical subproblems.

A recursive function calls itself with smaller inputs. For example, a function f(n) can be defined recursively as:
f(n) = 3 * f(n - 1) for all natural numbers n greater than zero, with the base case f(0) = 5.

Another classic recurrence relation is the Fibonacci sequence, where:
...
The Pigeonhole Principle states that if k + 1 or more objects are placed into k boxes, then at least one box must contain two or more objects. 

The generalized version states that if N objects are placed into k boxes, then at least one box must contain at least ceiling(N / k) objects.

In software engineering, the Pigeonhole Principle is used to prove the inevitability of collisions. For example, in hash functions, because the set of possible input keys is larger than the set of available hash table slots, collisions are mathematically guaranteed to occur. Engineers must design collision-resolution strategies (such as chaining or open addressing) to handle these occurrences. It is also used to prove bounds on compression algorithms, showing that no compression algorithm can compress all possible inputs. If a compression utility reduces the size of some files, it must inevitably expand others, a mathematical constraint derived from counting mappings between finite sets.
