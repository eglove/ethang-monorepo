...
export const mathematicalFoundationsDiscreteProbability = defineRule({
  content: `# Mathematical Foundations - Discrete Probability
...
2. **Probability Assignment**: A rule that assigns a real number between zero and one to each event in the sample space. An event is a subset of the sample space representing a possible outcome or a set of outcomes.
...
2. The sum of all probabilities over the entire support of the random variable must equal exactly one: sum of P(x) = 1.
