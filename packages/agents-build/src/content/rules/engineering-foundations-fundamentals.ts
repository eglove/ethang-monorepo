...
export const engineeringFoundationsFundamentals = defineRule({
  content: `# Engineering Foundations - The Engineering Process
...
2. **Defining Selection Criteria**: Decisions in engineering must be based on objective, quantifiable criteria. These criteria represent the multi-dimensional value space of the project, including financial limits (cost, return on investment), resource availability, time-to-market, regulatory requirements, safety tolerances, and quality attributes (performance, reliability, maintainability).
3. **Identifying Technically Feasible Solutions**: The optimal solution is rarely the first one that comes to mind. To avoid local design maxima, engineers must systematically generate and consider multiple reasonable, technically feasible solutions. This requires a broad knowledge of the domain and the discipline's state of the art.
4. **Evaluating Solutions Against Selection Criteria**: Each candidate solution must be rigorously analyzed and evaluated against the predefined criteria. This evaluation often involves mathematical modeling, computer simulation, prototyping, and cost-benefit analysis.
5. **Selecting the Preferred Option**: Once candidate solutions are evaluated, engineers use structured decision-making techniques to choose the alternative that best satisfies the criteria while balancing trade-offs.
6. **Monitoring Real-World Performance**: Because engineering decisions depend on estimates, projections, and models, they are subject to error. Continuous monitoring of the selected solution's real-world performance is essential to validate design assumptions and determine if alternative designs or corrections are necessary.
...
2. **Feedback for Iteration**: Identifying performance gaps, unexpected failure modes, or shifting requirements that trigger subsequent iterations of the engineering process.
