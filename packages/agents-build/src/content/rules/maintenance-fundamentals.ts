...
export const maintenanceFundamentals = defineRule({
  content: `# Software Maintenance Fundamentals
...
2. **Increasing Complexity**: As a system evolves, its complexity increases unless active work (such as refactoring) is done to simplify it.
3. **Self-Regulation**: The evolution process exhibits a self-regulating behavior with statistically determinable trends and indicators.
4. **Organizational Stability**: The average activity rate in an evolutionary process is constant over a project's lifetime, regardless of resource changes.
5. **Conservation of Familiarity**: The content of successive releases remains statistically constant to prevent user disruption and maintain understanding.
6. **Continuing Growth**: The functional content of a system must grow continually to maintain user satisfaction over time.
7. **Declining Quality**: System quality will appear to decline unless the software is actively adapted to its changing environment.
8. **Feedback System**: Evolutionary processes constitute multi-loop, multi-agent feedback systems and must be managed as such to control drift.
