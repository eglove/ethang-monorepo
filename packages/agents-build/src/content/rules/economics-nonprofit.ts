...
export const economicsNonprofit = defineRule({
  content: `# Software Engineering Economics: Nonprofit Decision-Making
...
2. Estimating the life-cycle costs and public benefits for each alternative over the designated planning horizon.
3. Discounting all costs and benefits to their present value using the approved social discount rate.
4. Ordering the alternatives by ascending present value of costs. The baseline (do-nothing) option is considered the starting point.
5. Computing the baseline BCR for the lowest-cost alternative. If its BCR is less than 1.0, it is discarded. If it is greater than or equal to 1.0, it becomes the current best option.
6. Comparing the next higher-cost alternative to the current best option by calculating the incremental BCR.
7. If the incremental BCR is greater than or equal to 1.0, the higher-cost alternative is accepted as the new current best option. If not, the higher-cost alternative is discarded.
8. Repeating the comparison for all remaining alternatives. The final current best option represents the economically optimal choice.
