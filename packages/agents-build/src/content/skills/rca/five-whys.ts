...
export const rcaFiveWhysSkill = defineSkill({
  content: `# Bug Root Cause Analysis
...
> This skill contains only the repo-specific procedures and output contract.
...
to assert) and the implementation plan (what to change).`,
  description:
    "Bug root cause analysis: trust-the-problem discipline, the 5-Whys chain, defect-pattern scan across the repo, regression analysis via git log, and defect classification. Use when the task is a bug or contains 'defect'/'regression'/'fix'; produces the RCA_FINDINGS contract.",
  name: "rca-five-whys"
});
