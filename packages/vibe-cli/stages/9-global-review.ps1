function Invoke-GlobalReview {
    ...
    for ($globalRound = 1; $globalRound -le $Config.MaxGlobalFixRounds; $globalRound++) {
        ...
        $globalReview = Invoke-ReviewRunner `
            ...
            -Context "Global review — full integration branch"
        ...
        # Verify fixes
        $lintOk = pnpm lint 2>&1; $lintPass = $LASTEXITCODE -eq 0
        $testOk = pnpm test 2>&1; $testPass = $LASTEXITCODE -eq 0
        $tscOk  = pnpm tsc 2>&1;  $tscPass  = $LASTEXITCODE -eq 0
