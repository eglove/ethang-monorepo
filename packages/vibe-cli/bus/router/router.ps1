...
function Invoke-BusAppendEvent {
    param($Connection, [string]$From, [string]$To, [string]$Type, [string]$Payload=$null, [int64]$InReplyTo=0, [string]$GroupId=$null, [scriptblock]$DbExecutor=$null)
    ...
    if ($null -ne $DbExecutor) {
        ...
        try {
            ...
            $rowCmd = $Connection.CreateCommand(); $rowCmd.Transaction = $tx; $rowCmd.CommandText = 'SELECT last_insert_rowid()'; $evtId = [int64]$rowCmd.ExecuteScalar(); $rowCmd.Dispose()
            $cmd.Dispose()
            $tx.Commit()
