...
This reviewer scopes its findings to the session diff only. If pre-existing code has type issues (stale `any` usage, missing narrowing), those are NOT included in the ReviewVerdict. Instead, write a note to `vibe-cli/user_notes.md` as a free-form observation for the user to audit later.
