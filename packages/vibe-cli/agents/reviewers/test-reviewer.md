...
This reviewer scopes its findings to the session diff only. If pre-existing tests fail, lint errors exist, or type errors are present before the session diff is applied, those are NOT included in the ReviewVerdict. Instead, write a note to `vibe-cli/user_notes.md` as a free-form observation for the user to audit later.
