...
describe("createJsonResponse", () => {
  ...
  it("should handle undefined data correctly", () => {
    const response = createJsonResponse(null, "INTERNAL_SERVER_ERROR");
  ...
  it("should remove invalid headers", () => {
    ...
    expect(publicKeyPins).toBeNull();
    expect(aspNetVersion).toBeNull();
  ...
  describe("atomic header copying and security header enforcement (SR-3)", () => {
    ...
    it("should enforce critical security headers (CSP, CORS, X-Frame-Options) and prevent deletion via null overrides", () => {
      const response = createJsonResponse({ ok: true }, "OK", {
        headers: {
          "Access-Control-Allow-Origin": null as any,
          "Content-Security-Policy": null as any,
    ...
    it("should support headers passed as an array of entries", () => {
      ...
      expect(response.headers.get("X-Another-Array-Header")).toBe(
        "AnotherValue"
      );
    });

    it("should delete non-critical headers when set to null", () => {
      const response = createJsonResponse({ ok: true }, "OK", {
        headers: {
          "X-Content-Type-Options": null as any,
