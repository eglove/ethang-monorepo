...
7. **Improve, Don't Just Replicate**: A working pattern is a starting point, not a ceiling. "That's how this repo does it" is not a reason to copy a weaker design. When modifying existing code — feature, bugfix, or test — scan the surrounding module for structural improvements: duplicated logic extractable into a shared utility, functions grown too large that should be split, conditionals simplifiable via polymorphism or pattern matching, and test gaps that can be filled. When creating something new, choose the cleaner design over the familiar one — for example, proper DDD built on Effect (aggregates, value objects, repository ports as `Context.Tag`, use cases as `Effect.gen`, dependency injection via `Layer`) rather than "DDD-ish" `getDb()` threading. Every change should be an overall improvement over the baseline, and improvements should stay visible and backportable to sibling apps rather than becoming one-offs. Scope refactors to the module being changed; do not expand into unrelated files. Run the test suite before and after to confirm no behavior change.
