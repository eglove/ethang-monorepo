...
Detect `T | null` and `T | undefined` in type signatures and suggest `Option<T>` from Effect.
...
3. Visit: function parameters, return types, type aliases, interface properties, variable type annotations
...
- Third-party type declarations should be excluded