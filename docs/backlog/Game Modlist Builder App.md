...
Build a web application for building game mod lists. Users search for mods, add them to a list, and the system resolves requirements, conflicts, and patches.
...
2. **Data model**: Design schema for mods, dependencies, conflicts, patches, modlists
3. **MVP**: Search + add to list + basic conflict detection
4. **Graph view**: Visual dependency/conflict graph
5. **Resolution engine**: Auto-suggest patches and load order
6. **Export**: Multiple export formats

## Open Questions

- Which game(s) first? Skyrim? Fallout? Starfield?
- Data source: scrape or API? (Nexus Mods has no public API)
- Is this a standalone app or part of the monorepo?
- Auth needed? (save/share modlists)
