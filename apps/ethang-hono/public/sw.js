...
var q = (o, e, t) =>
  t.some((s) => o.headers.has(s) && e.headers.has(s))
    ...
    : !0;
var B = "CACHE_UPDATED",
  K = "workbox-broadcast-update",
  Y = !0,
  Q = ["content-length", "etag", "last-modified"];
var te = /^((?!chrome|android).)*safari/i.test(navigator.userAgent);
function re(o) {
...
var y = class {
  constructor({
    ...
    ((this._headersToCheck = t || Q),
      (this._generatePayload = e || re),
      (this._notifyAllClients = r ?? Y));
  }
  async notifyIfUpdated(e) {
    if (
      e.oldResponse &&
      !q(e.oldResponse, e.newResponse, this._headersToCheck)
    ) {
      let t = { type: B, meta: K, payload: this._generatePayload(e) };
      if (e.request.mode === "navigate") {
        let r;
        (e.event instanceof FetchEvent && (r = e.event.resultingClientId),
          (!(await A(r)) || te) && (await f(3500)));
      }
...
var b = class {
  constructor(e) {
    ((this.cacheDidUpdate = async (t) => {
      L(this._broadcastUpdate.notifyIfUpdated(t));
    }),
      (this._broadcastUpdate = new y(e)));
  }
};
var P = new Set();
var m = {
  ...
  V = (o) => [m.prefix, o, m.suffix].filter((e) => e && e.length > 0).join("-"),
  oe = (o) => {
  ...
  R = {
    ...
    getGoogleAnalyticsName: (o) => o || V(m.googleAnalytics),
    getPrecacheName: (o) => o || V(m.precache),
    getPrefix: () => m.prefix,
    getRuntimeName: (o) => o || V(m.runtime),
    getSuffix: () => m.suffix,
  };
function J(o, e) {
  let t = new URL(o);
...
var C,
  D = () => (
    C || ((C = new k()), C.addFetchListener(), C.addCacheListener()),
    C
  );
function U(o, e, t) {
  let r;
  if (typeof o == "string") {
    ...
    r = new p(a, e, t);
  } else if (o instanceof RegExp) r = new v(o, e, t);
...
var $ = class {
  ...
  async cachePut(e, t) {
    ...
    let { cacheName: a, matchOptions: i } = this._strategy,
      ...
      d = E ? await W(l, s.clone(), ["__WB_REVISION__"], i) : null;
    try {
...
var h = class {
  ...
  handleAll(e) {
    ...
    let t = e.event,
      ...
      n = new $(this, { event: t, request: r, params: s }),
...
self.addEventListener("activate", (o) => {
  ...
  o.waitUntil(
    caches
      ...
      .then(async (t) =>
        Promise.all(
          t.filter((r) => !e.has(r)).map(async (r) => caches.delete(r)),
        ),
      ),
  );
});
U(
  ...
  new w({
    cacheName: I,
    plugins: [new b({ headersToCheck: ["etag", "last-modified"] })],
  }),
);
U(
...
self.addEventListener("message", (o) => {
  ...
  let r = "type" in e && e.type === "PRECACHE_LINKS",
    s = "urls" in e && Array.isArray(e.urls);
  if (r && s) {
    let n = e.urls;
    o.waitUntil(caches.open(I).then(async (a) => a.addAll(n)));
  }
});
