...
describe("course-completion.client setPercentages tests", () => {
  it("handles all status types and small percentages", async () => {
    ...
    document.body.innerHTML = `
      ...
      ${Array.from({ length: 96 }, (_, index) => {return `<div><button class="course-completion-button" data-course-url="url-${index + 5}"></button><div class="course-status-text">Complete</div></div>`}).join("")}
    `;
