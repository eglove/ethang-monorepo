...
test.describe("scroll-containers", () => {
  test("scroll-containers page loads and passes a11y", async ({ axePage }) => {
    ...
    await expect
      .soft(axePage.getByRole("heading", { name: "Easy Sticky Header/Footer" }))
      .toBeVisible();
  });
  ...
  test("scroll-containers page shows CSS and Tailwind code examples", async ({
    ...
    await expect
      .soft(axePage.getByRole("heading", { name: "CSS" }))
      .toBeVisible();
    await expect
      .soft(axePage.getByRole("heading", { name: "Tailwind" }))
      .toBeVisible();
    await expect
      .soft(axePage.getByRole("heading", { name: "Demo" }))
      .toBeVisible();
  });
});
