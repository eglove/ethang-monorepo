...
test.describe("sign in", () => {
  ...
  test("sign-in form has accessible email and password fields", async ({
    ...
    await expect.soft(axePage.getByLabel("Password")).toBeVisible();
    await expect
      .soft(axePage.getByRole("button", { name: "Submit" }))
      .toBeVisible();
  });
  ...
  test.describe("sign-in form — submission behavior", () => {
    test("successful sign-in sets auth cookie and redirects to /courses", async ({
      ...
      await axePage.goto(routes.signIn, { waitUntil: "load" });
      ...
      const authCookie = find(cookies, (c) => {
        return "ethang-auth-token" === c.name;
      });
    ...
    test("failed sign-in shows error message", async ({ axePage }) => {
      ...
      const errorElement = axePage.locator(SIGN_IN_ERROR);
