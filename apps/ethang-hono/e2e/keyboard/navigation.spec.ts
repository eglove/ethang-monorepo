...
test.describe("navigation hamburger menu — keyboard user (mobile)", () => {
  test("hamburger button is focusable and toggles menu with Enter", async ({ page, isMobile }) => {
    test.skip(!isMobile, "Hamburger menu only appears on mobile viewports");

    await page.goto("/");

    const hamburgerButton = page.getByRole("button", { name: HAMBURGER_BUTTON_NAME });
  ...
  test("hamburger button toggles menu with Space", async ({ page, isMobile }) => {
    test.skip(!isMobile, "Hamburger menu only appears on mobile viewports");
