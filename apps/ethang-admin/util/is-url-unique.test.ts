/* eslint-disable @typescript-eslint/no-unsafe-argument, @typescript-eslint/no-unsafe-type-assertion, @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-return, @typescript-eslint/no-explicit-any */
...
describe("isUrlUnique validation rule", () => {
  ...
  it("should handle null ISO date fallback", async () => {
    vi.spyOn(DateTime, "now").mockReturnValue({
      toISODate: vi.fn().mockReturnValue(null)
    } as any);
    ...
    expect(result).toBe(true);
    expect(mockGetClient).toHaveBeenCalledWith({ apiVersion: null });
  });
});
