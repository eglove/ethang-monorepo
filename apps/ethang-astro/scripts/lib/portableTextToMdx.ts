...
const inline = (spans: unknown, markDefs: unknown): string => {
	...
	for (const raw of (spans as unknown[] | undefined) ?? []) {
		...
		const href =
			annot?._type === "link" && isString(annot.href) ? annot.href : undefined;
...
export const portableTextToMdx = (
	...
	const asObj = (b: unknown): Record<string, unknown> =>
	...
	for (const blockRaw of blocks ?? []) {
		...
		if (type === "block") {
			const listItem = isString(block["listItem"])
				? (block["listItem"] as string)
				: null;
		...
		if (type === "image") {
			...
			const relative = resolveImage(
				isString(asset["url"]) ? (asset["url"] as string) : undefined,
			);
			if (!relative) continue;
			const alt = isString(block["alt"]) ? (block["alt"] as string) : "";
			const caption = isString(asset["caption"])
				? (asset["caption"] as string)
				: "";
			if (caption) {
				const variable = `img${images.length}`;
				images.push({ variable, src: relative, alt, caption });
				body.push(
					`<PostImage src={${variable}} alt="${alt.replace(/"/g, "&quot;")}" caption="${caption.replace(/"/g, "&quot;")}" />`,
				);
			} else {
			...
			for (const key of ["videoId", "url", "title"]) {
				if (isString(block[key]) && block[key])
					attrs.push(
						`${key}="${(block[key] as string).replace(/"/g, "&quot;")}"`,
					);
			}
			if (attrs.length) {
				body.push(`<VideoEmbed ${attrs.join(" ")} />`);
				body.push("");
			}
			...
			body.push(
				`<Blockquote${attrs.length ? " " + attrs.join(" ") : ""}>${escapeMarkdown(quote)}</Blockquote>`,
			);
			body.push("");
		}
	}
