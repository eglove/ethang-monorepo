...
const NOW = DateTime.fromISO("2024-06-15T14:00:00.000Z");
...
describe(getIsHappeningNow, () => {
  ...
  it("returns true when now is exactly at start", () => {
    ...
    expect(getIsHappeningNow(NOW.toString(), "2024-06-15T16:00:00.000Z")).toBe(
      true
    );
  ...
  it("returns true when now is exactly at end", () => {
    ...
    expect(getIsHappeningNow("2024-06-15T13:00:00.000Z", NOW.toString())).toBe(
      true
    );
