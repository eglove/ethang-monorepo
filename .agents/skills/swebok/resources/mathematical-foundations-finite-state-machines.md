# Mathematical Foundations - Finite State Machines
...
1. **I** is a finite set of input symbols, forming the input alphabet.
1. **O** is a finite set of output symbols, forming the output alphabet.
1. **f** is the state transition function, mapping the current state and an input symbol to the next state: f: S x I -> S.
1. **g** is the output function, mapping the state and input to an output: g: S x I -> O.
1. **s0** is the designated initial state (or start state) where the machine starts its execution, such that s0 is an element of S.
