...
1. **Lexical Analysis**: Groups characters into meaningful tokens (keywords, identifiers, operators).
1. **Syntax Analysis**: Parses tokens into a hierarchical structure, such as a parse tree or abstract syntax tree (AST).
1. **Intermediate Code Generation**: Produces a machine-independent representation of the program to facilitate optimization.
1. **Optimization**: Restructures intermediate code to improve execution speed or reduce memory footprint.
1. **Code Generation**: Translates optimized intermediate code into target machine instructions.
1. **Linkers and Loaders**: Linkers resolve external references. Loaders copy the final executable into physical memory.
