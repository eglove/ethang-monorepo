...
1. **Non-Runtime Observable Qualities**: Static properties that cannot be measured during execution but govern how easily the system can be evolved and maintained (e.g., modifiability, portability to new runtimes, reusability, testability).
1. **Design-Observable Qualities**: Structural characteristics that can be directly evaluated by inspecting the design specifications (e.g., conceptual integrity—ensuring the codebase feels like it was designed by a single mind—correctness, completeness, coupling levels).
