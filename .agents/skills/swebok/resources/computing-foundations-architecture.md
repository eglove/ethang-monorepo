# Computing Foundations - Computer Architecture and Organization
...
1. **Data Bus**: Serves as the bi-directional pathway for transferring data bits to and from the specified memory location or I/O device.
1. **Control Bus**: Carries synchronization and control signals from the CPU to external components (including read/write controls, device enables, interrupts, status lines, and reset signals).
